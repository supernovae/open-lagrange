...
export async function handleCreateRun(raw: unknown): Promise<unknown> {
  ...
  if (payload.source === "builder_session") {
    ...
    return checkAndCreateRunFromBuilderSession({ session_id: payload.session_id, live: payload.live, ...(runtimeProfile ? { runtime_profile: runtimeProfile } : {}) });
  }
  ...
  return checkAndCreateRunFromPlanfile({ planfile, live: payload.live, ...(runtimeProfile ? { runtime_profile: runtimeProfile } : {}) });
}
