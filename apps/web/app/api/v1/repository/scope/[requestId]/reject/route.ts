...
import { handleRejectRepositoryScopeRequest } from "../../../../handlers";
...
export async function POST(request: Request, context: { readonly params: Promise<{ readonly requestId: string }> }): Promise<Response> {
  try {
    ...
    return json(await handleRejectRepositoryScopeRequest(requestId, await parseJson(request, z.unknown())), { status: 202 });
  } catch (error) {
