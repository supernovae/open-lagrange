...
const researchTopicBriefTemplate = PlanTemplate.parse({
  ...
  nodes_template: [
    ...
    templateNode("fetch_sources", "inspect", "Fetch selected sources", "Fetch the selected source set through policy and SDK HTTP primitives.", ["select_sources"], "research.fetch_sources", ["Source snapshots"], { sources: "$nodes.select_sources.output.selected_sources", mode: "live" }),
    templateNode("extract_sources", "analyze", "Extract source content", "Extract readable text and citation metadata from fetched sources.", ["fetch_sources"], "research.extract_sources", ["Extracted sources"], { fetched_sources: "$nodes.fetch_sources.output.fetched_sources" }),
    templateNode("create_source_set", "analyze", "Create source set", "Create a cited source set from extracted sources.", ["extract_sources"], "research.create_source_set", ["Source set"], { topic: "$parameters.topic", sources: "$nodes.extract_sources.output.sources", selection_policy: { max_sources: "$parameters.max_sources", prefer_official: true, require_diverse_domains: true } }),
    templateNode("create_brief", "analyze", "Create cited brief", "Use the configured model provider to synthesize a cited Markdown brief across extracted sources.", ["create_source_set"], "research.create_brief", ["Research brief"], { topic: "$parameters.topic", source_set_id: "$nodes.create_source_set.output.source_set_id", sources: "$nodes.extract_sources.output.sources", brief_style: "$parameters.brief_style", include_recommendations: false, max_words: 800 }),
    templateNode("export_markdown", "finalize", "Export Markdown brief", "Write the brief as an indexed Markdown artifact.", ["create_brief"], "research.export_markdown", ["Markdown artifact"], { title: "$parameters.title", markdown: "$nodes.create_brief.output.markdown", related_source_ids: "$nodes.create_source_set.output.selected_sources.source_id" }),
  ],
});
