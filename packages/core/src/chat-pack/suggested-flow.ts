...
export function flowForResearchBrief(topic: string): SuggestedFlow {
  return SuggestedFlow.parse({
    ...
    summary: "Create a cited research brief from live search when configured, or ask for explicit source URLs.",
    command: `/research brief ${quote(topic)}`,
    event: { type: "research.brief", topic, mode: "live" },
    side_effects: ["Writes research artifacts after confirmation."],
    required_packs: ["open-lagrange.research"],
    approval: "If live search is not configured, the command yields with remediation instead of using fixtures.",
    confidence: "high",
    requires_confirmation: true,
  });
}
